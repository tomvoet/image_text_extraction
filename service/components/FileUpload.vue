...
const onDrop = (file: File[] | null | FileList) => {
    if (file) {
        data.value = file[0]
    }
}
...
const { isOverDropZone } = useDropZone(dropZoneRef, { onDrop, dataTypes: ["image/*"] })
...
<template>
    <div class="flex items-center justify-center w-full">
        <label for="file-dropzone" ref="dropZoneRef"
            ...
            <input id="file-dropzone" accept="image/*" type="file" class="hidden" multiple="false" @change="onUpdate" />
        </label>
    </div>
</template>
