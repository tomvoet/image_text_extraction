...
const downloadTxt = () => downloadTxtFile(text.value)
...
<template>
    <div v-if="text" class="relative">
        <UTextarea v-model="text" autoresize textareaClass="p-8" rows="1" />
        <div
            class="absolute flex flex-row top-0 right-0 bg-gray-100 dark:bg-gray-800 px-2 rounded-bl rounded-tr-md border-r border-t dark:border-gray-700 border-gray-300">
            <UButton @click="downloadTxt" size="sm" variant="ghost" color="gray" class="p-0 my-1 hover:bg-transparent"
    ...
    <div v-else-if="loading">
        <div class="flex items-center justify-center w-full h-32">
            ...
            <div class="ml-4 text-gray-500 dark:text-gray-400">Extracting text...</div>
        </div>
    </div>
</template>